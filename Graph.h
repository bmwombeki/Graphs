...
public:
    ...
    struct vertex_label {
        ...
        char state;
        int npaths;
        int numberOfOutgoing;
        int numberOfIncoming;
        
        vertex_label( double _dist=0.0, int _pred=-1, char _state='?',
                     int _npaths=0)
        : dist { _dist }, pred { _pred }, state { _state}, npaths { 0 },numberOfOutgoing{0},numberOfIncoming{0}
        { }
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
        ...
        while(!q.empty()) {
            ...
            for(edge &e : vertices[u].outgoing) {
                ...
                //keeping track of the number of paths
                // and recording the number of shortest paths to vertex
                if(report[v].state == DISCOVERED) {
                    if( report[u].dist+1 == report[v].dist){
                        report[v].npaths = report[v].npaths + report[u].npaths;
                        
                    } // end of if ( report[u].dist+1 == report[v].dist)
...
public:
    ...
    bool extract_path(const vector<vertex_label> & rpt,
        ...
        //check if there no path ending at index dest
        if(rpt[dest].pred == -1) {
            return false;
        }
        ...
        helperFunctionForextract_path(rpt, path, dest); // recursively add vector in the order
...
private:
    ...
    void helperFunctionForextract_path(const vector<vertex_label> &rpt, vector<int> &path, int dest ) {
        ...
        helperFunctionForextract_path(rpt,path, rpt[dest].pred);
        path.push_back(dest);
...
public:
    bool dag_critical_paths(vector<vertex_label> & rpt) {
        ...
        vector<int>newVector; // this vector will be populated with topo_sort
        topo_sort(newVector); // sort the vector
        
       int i = 0;
        int theSize = newVector.size();
        while (i < theSize)
        {
            if(vertices[newVector[i]].incoming.size() == 0) {
                rpt[newVector[i]].pred = newVector[i];
                rpt[newVector[i]].dist = 0;
            } // end of if statement
            
            for(edge &e : vertices[newVector[i]].outgoing){
                int z = e.vertex_id;
                double theWeight = e.weight;
                if( rpt[newVector[i]].dist +  theWeight > rpt[z].dist ) {
                    rpt[z].dist = rpt[newVector[i]].dist + theWeight;
                    rpt[z].pred = newVector[i];
                }
...
private:
    int  nodesIncoming(vector<vertex_label> &rpt, int number) {
        if((vertices[number].incoming.size()) == 0){  // check if the incoming node is 0
            rpt[number].numberOfIncoming = 1;
        }
        
       else {
            // go through the edges
            int k = 0;
           double theSize = vertices[number].incoming.size();
           while (k < theSize)
            {
                int n = vertices[number].incoming[k].vertex_id;
                if(0 < rpt[n].numberOfIncoming){
                    rpt[number].numberOfIncoming += rpt[n].numberOfIncoming;
                    rpt[number].npaths = rpt[number].numberOfIncoming;
                }
                // call next one recursively
                else {
                    
                    rpt[number].numberOfIncoming += nodesIncoming(rpt,n);
                }
                k++; // increment k
            } // enf of while loop
        ...
        return rpt[number].numberOfIncoming;
        
    }// end of nodesIncoming
    
    
    int  nodesOutgoing(vector<vertex_label> &rpt, int number) {
        if((vertices[number].outgoing.size()) == 0){ // check if the incoming node is 0
            rpt[number].numberOfOutgoing = 1;
        }
        
        else {
            // go through the edges
            int k = 0;
            double theSize = vertices[number].outgoing.size();
            while (k < theSize)
            {
                int n = vertices[number].outgoing[k].vertex_id;
                
                if( 0 < rpt[n].numberOfOutgoing ){
                    rpt[number].numberOfOutgoing += rpt[n].numberOfOutgoing;
                    rpt[number].npaths = rpt[number].numberOfOutgoing;
                }
                // call next one recursively
                else {
                    
                    rpt[number].numberOfOutgoing += nodesOutgoing(rpt,n);
                }
                k++; // increment k
            } // end of while loop
        ...
        return rpt[number].numberOfOutgoing;
...
public:
    bool dag_num_paths(vector<vertex_label> & rpt) {
        ...
        // getting outgoing number of edges
        while(j< size) {
            if((vertices[j].incoming.size()) == 0) {
                nodesOutgoing(rpt, j);
            }
              j++;
        } // end of while loop
        
        // getting incoming number of edges
        while (i < size) {
        ...
        while(k < size) {
            rpt[k].npaths = rpt[k].numberOfIncoming * rpt[k].numberOfOutgoing;
            k++;
        }
        
        return true;
        
    } // end of dag_num_paths
...
private:
    ...
    bool valid_topo_order(const vector<int> & order) {
        ...
        vector<topOrder> trace;
        
        trace.resize(order.size()); // resizing the vector
        //loop though the entire graph
          int num;
        
        for(int l = 0;l < order.size(); l++){
            // check if there is no incoming vertices, and if there isn't mark it on the trace vector
            if(vertices[order[l]].incoming.size() == 0){
                trace[order[l]].numberAppear = - 999; // this means it is processed
                continue;
            }
            for (int n = 0; n < vertices[order[l]].incoming.size(); n++) {
                num = vertices[order[l]].incoming[n].vertex_id;
                if(trace[num].numberAppear != -999) {
                    return false;
                }
            }
            
            // making the current position to the vector
            
            trace[order[l]].numberAppear = -999;
          
        } // end of for loop...
        
        return true;
        
    } // end of valid_topo_order....
    ...
    bool enum_paths(int target, vector<string> &paths) {
        ...
        while(1) {
            ...
            for (int i=0; i < results.size(); i++) {
                vector<int> path = results[i];
                int node = path[path.size()-1];
                for (int k =0; k < vertices[node].incoming.size(); k++) {
                    cnt++;
                    vector<int> path2 = path;
                    path2.push_back(node);
                    vecTemp.push_back(path2);
                }
                if(vertices[node].incoming.size()==0)
                    vecTemp.push_back(path);
            }
            if(cnt == 0)
                break;
            results = vecTemp;
        ...
        paths.push_back(id_vec2string(results[j]));
        std::cout <<id_vec2string(results[j]);
